...
### [x] P3-13 — Trigger and open child pipelines
...
- [x] Before writing production code, create
  ...
  accessibility, and non-goals.
- [x] Write failing endpoint, decoding, service, model, projection, and
...
- [x] Add the typed bridge Play mutation through the existing pipeline action
  boundary. Send each POST once and never create an unrelated pipeline as a
  fallback.
- [x] Reconcile the authoritative bridge, refresh bounded parent/trigger
  reads, and make the row natively navigable when GitLab supplies valid
  downstream identity.
- [x] Add one compact trailing Liquid Glass Play control for manual trigger
  ...
  Simulator.
- [x] Run focused pipeline tests, existing ordinary action tests, a Release
  Simulator build, Xcode static analysis, diff checks, and
  credential/privacy scans.
- [x] Perform a deep code review of all P3-13 code for bugs, bad code,
